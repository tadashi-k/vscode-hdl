...
class VerilogSymbolVisitor extends VerilogVisitor {
     ...
     * are generated by referring to the moduleDatabase argument.
     * If an instanced module name is not in moduleDatabase, a 'module not found'
     * warning is shown.
...
class AntlrVerilogParser {
     ...
     */
    static evaluatePortWidth(port: any, moduleDefaultParams: any[], overrides: Record<string, number> | null): number | null {
    ...
    /**
     * Internal method: preprocess, lex, parse, and visit a Verilog document.
     * Returns the VerilogSymbolVisitor populated with module/signal/instance data,
     * or null if a fatal error occurred.
     *
     * As a side effect, this.errorListener is populated with syntax errors
     * and this._lastVisitor is stored for subsequent use.
     */
    _parse(document: any, fileReader: ((resolvedPath: string) => string | null) | null = null): VerilogSymbolVisitor | null {
    ...
    /**
     * Parse Verilog document and return modules with only parameter and port
     * information (no signals or instances).
     *
    ...
    parseModules(document: any, fileReader: ((resolvedPath: string) => string | null) | null = null): Module[] {
        const visitor = this._parse(document, fileReader);
        if (!visitor) return [];

        // Return modules with only parameters and ports (strip signals and instances)
        return visitor.modules.map(mod => {
            const stripped = new Module(mod.name, mod.uri, mod.line, mod.character, mod.scanned);
            stripped.startLine = mod.startLine;
            stripped.endLine = mod.endLine;
            stripped.ports = mod.ports;
            stripped.parameterList = mod.parameterList;
            stripped.parameterMap = mod.parameterMap;
            return stripped;
        });
    }
     ...
     * Returns an array of fully-populated Module objects with signals,
     * parameters, instances, and ports.
     *
     * @param {vscode.TextDocument} document
     * @param {Function} [fileReader] - Optional file reader for `include resolution
    ...
    parseSymbols(document: any, fileReader: ((resolvedPath: string) => string | null) | null = null): Module[] {
        ...
        const moduleObjects: Module[] = visitor.modules;
     ...
     * Calls parseSymbols internally to perform the parse, then pulls errors
     * from VerilogErrorListener and warnings from VerilogSymbolVisitor.
     *
     * @param {vscode.TextDocument} document
     * @param {ModuleDatabase} [moduleDatabase] - Optional workspace-wide module database for cross-file port lookup
     * @param {Function} [fileReader] - Optional file reader for `include resolution
